...
BUILD_DIR="${LITERT_RISCV64_BUILD_DIR:-${REPO_ROOT}/build-riscv64}"
SOURCE_DIR="${REPO_ROOT}/docker_build/riscv64_smoke"
TOOLCHAIN_FILE="${REPO_ROOT}/docker_build/cmake/riscv64-linux-gnu.cmake"
TARGET="${LITERT_RISCV64_TARGET:-litert_riscv64_smoke}"
...
usage() {
  cat <<'EOF'
Usage: docker_build/riscv64_build_and_test.sh [options]

Options:
  --target NAME          CMake target to build. Default: litert_riscv64_smoke
  --build-dir PATH      Build directory. Default: ./build-riscv64
...
echo "[riscv64] Repository: ${REPO_ROOT}"
echo "[riscv64] Build dir:  ${BUILD_DIR}"
echo "[riscv64] Target:     ${TARGET}"
echo "[riscv64] ISA:        ${RISCV64_MARCH}/${RISCV64_MABI}"
echo "[riscv64] RVV:        ${TFLITE_ENABLE_RVV}"
...
ARTIFACT=""
if [[ "${TARGET}" == "litert_riscv64_smoke" ]]; then
  ARTIFACT="${BUILD_DIR}/litert_riscv64_smoke"
...
if [[ -z "${ARTIFACT}" || ! -f "${ARTIFACT}" ]]; then
  echo "[riscv64] Built target '${TARGET}', but no executable artifact was found for QEMU." >&2
  echo "[riscv64] Build output remains in ${BUILD_DIR}" >&2
  exit 0
fi

echo "[riscv64] Artifact: ${ARTIFACT}"
file "${ARTIFACT}"
riscv64-linux-gnu-readelf -h "${ARTIFACT}" | sed -n '/Class:/p;/Machine:/p;/Flags:/p'

if ! file "${ARTIFACT}" | grep -q "RISC-V"; then
  echo "[riscv64] Artifact is not a RISC-V executable: ${ARTIFACT}" >&2
  exit 1
fi

if [[ "${RUN_QEMU}" -eq 1 ]]; then
  echo "[riscv64] Running QEMU smoke test..."
  qemu-riscv64 -L /usr/riscv64-linux-gnu -cpu "${QEMU_CPU}" "${ARTIFACT}"
fi

echo "[riscv64] Build and smoke test completed."
